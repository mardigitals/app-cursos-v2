import { Injectable, NotFoundException, BadRequestException } from '@nestjs/common'; // <-- 1. BadRequestException importado
...
    private readonly notaRepository: Repository<Nota>,
    ...
    // --- 3. INYECCIÓN AÑADIDA ---
    // (Necesaria para poder validar el estado de la inscripción padre)
    @InjectRepository(Inscripcion)
    private readonly inscripcionRepository: Repository<Inscripcion>,
  ) {}

  // --- 4. MÉTODO 'create' ACTUALIZADO CON VALIDACIÓN ---
  async create(createDto: CreateNotaDto): Promise<Nota> {
    ...
    // 3. ¡VALIDACIÓN CLAVE! Verificamos el estado de la inscripción
    ...
    // Si todo está bien, creamos la nota
    const nota = this.notaRepository.create(createDto);
    return this.notaRepository.save(nota);
  }

  // 2. Obtener todas las notas (Sin cambios)
...
  // 3. Obtener una nota por ID (Sin cambios)
...
  // 4. Actualizar nota (Sin cambios)
  // (Podríamos añadir la misma validación aquí para impedir editar notas de inscripciones cerradas)
...
  // 5. Eliminar nota (borrado físico) (Sin cambios)
  // (Este borrado físico está bien para las notas)
